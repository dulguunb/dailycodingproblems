...
impl BSTNode {
    ...
    //     res
    // }
    pub fn search(&mut self, searched_val: i32) -> (bool,i32,i32){
        if self.val == searched_val{
            self.visited+=1;
            return (true,self.visited,self.amount)
        }
        return if self.val > searched_val {
            match self.left {
                None => {
                    return   (false,self.visited,self.amount)
                }
...
impl Solution {
    pub fn intersect(nums1: Vec<i32>, nums2: Vec<i32>) -> Vec<i32> {
        ...
        for i in 0 .. nums2.len(){
            let (exists,num_visit,amount) = tree1.search(nums2[i]);
            if exists && num_visit == amount{
                res.push(nums2[i])
            }
        }
        res
...
mod tests {
    ...
    fn test_search_insert() {
        ...
        assert_eq!(Solution::intersect(vec![1], vec![1]), vec![1]);
